...
transmissionNetwork_sim <- function(n_reps, parameters){
  ...
  setwd("~/Desktop") ## call has to be from location of .exe file or parameter read-in fails???
...
parameters <- data.frame(model_type = 2, R0 = 1.2, R0_SS = 15, prob_SS = 0.1, k = 1.1,
...
for(i in 1:1){
  if(i==1){
    tst <- transmissionNetwork_sim(n_reps = 1, parameters = parameters)
    tst$network_ID = i 
  }else{
    tmp <- transmissionNetwork_sim(n_reps = 1, parameters = parameters)
    tmp$network_ID = i 
    tst <- rbind(tst, tmp)
  }
